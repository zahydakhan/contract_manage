...
import { delTAGSStart } from '../../../../redux/config_tags/config_tags.actions';
...
const TagdelForm = ({ setOpen, row }) => {
    const [submitError, setSubmitError] = useState('');
    const classes = useStyles();
    const dispatch = useDispatch()

    const submittion = async (row) => {
        await dispatch(delTAGSStart(row));
    ...
    return (
        <Grid container
            ...
            <Grid item>
                <Typography className={classes.subtitle} variant='h6'>Click below to delete {row.name} Tag permanently</Typography>
