...
const useStyles = makeStyles((theme) => ({
  ...
  const body = (rows) => (
      <div style={modalStyle} className={classes.paper}>
          <DivdelForm setOpen={setOpen} row={rows} />
      </div>
  );
        ...
        <TableContainer className={classes.table1} component={Paper} style={{ marginTop: "1.5em" }}>
          <Table stickyHeader aria-label="sticky table" classes={{root: classes.customTable}}>
            ...
            <TableBody>
              ...
              {(searchResult ?  (divData.filter((cty) =>
      cty.name.toLowerCase().includes(searchResult.toLowerCase()))) : divData ).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                return (
                  <TableRow key={row.id}>
        ...
        <TablePagination
          ...
          component="div"
          count={(searchResult ?  (divData.filter((cty) =>
            cty.name.toLowerCase().includes(searchResult.toLowerCase()))) : divData ).length}
