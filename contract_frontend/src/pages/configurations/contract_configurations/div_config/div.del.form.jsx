...
import { delDIVStart } from '../../../../redux/config_div/config_div.actions';
...
const DivdelForm = ({ setOpen, row }) => {
    const [submitError, setSubmitError] = useState('');
    const classes = useStyles();
    const dispatch = useDispatch()

    const submittion = async (row) => {
        await dispatch(delDIVStart(row));
    ...
    return (
        <Grid container
            ...
            <Grid item>
                <Typography className={classes.subtitle} variant='h6'>Click below to delete {row.name} Division permanently</Typography>
